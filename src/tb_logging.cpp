...
void log_destination(const char *tag, const char *prefix, const UniversalMessage_Destination *dest) {
  ...
  switch (dest->which_sub_destination) {
    case UniversalMessage_Destination_domain_tag:
      LOG_DEBUG("  domain: %s", domain_to_string(dest->sub_destination.domain));
      break;
    case UniversalMessage_Destination_routing_address_tag:
      LOG_DEBUG(
          "  routing_address: %s",
          format_hex(dest->sub_destination.routing_address.bytes, dest->sub_destination.routing_address.size).c_str());
      break;
    default:
      LOG_DEBUG("  unknown sub_destination");
  }
}
...
void log_information_request(const char *tag, const VCSEC_InformationRequest *msg) {
  LOG_DEBUG("VCSEC_InformationRequest:");
...
void log_routable_message(const char *tag, const UniversalMessage_RoutableMessage *msg) {
  LOG_DEBUG("UniversalMessage_RoutableMessage:");
  LOG_DEBUG("  has_to_destination: %s", msg->has_to_destination ? "true" : "false");
  if (msg->has_to_destination) {
    log_destination(tag, "to_destination", &msg->to_destination);
  }

  LOG_DEBUG("  has_from_destination: %s", msg->has_from_destination ? "true" : "false");
  if (msg->has_from_destination) {
    log_destination(tag, "from_destination", &msg->from_destination);
  }

  LOG_DEBUG("  which_payload: %d", msg->which_payload);
  ...
  LOG_DEBUG("  has_signedMessageStatus: %s", msg->has_signedMessageStatus ? "true" : "false");
  if (msg->has_signedMessageStatus) {
    log_message_status(tag, &msg->signedMessageStatus);
  }

  LOG_DEBUG("  which_sub_sigData: %d", msg->which_sub_sigData);
  switch (msg->which_sub_sigData) {
    case UniversalMessage_RoutableMessage_signature_data_tag:
      ...
      break;
    default:
      LOG_DEBUG("  sub_sigData: unknown");
      break;
  }

  switch (msg->flags) {
    case UniversalMessage_Flags_FLAG_USER_COMMAND:
      LOG_DEBUG("  flags: FLAG_USER_COMMAND");
      break;
    case UniversalMessage_Flags_FLAG_ENCRYPT_RESPONSE:
      LOG_DEBUG("  flags: FLAG_ENCRYPT_RESPONSE");
      break;
    default:
      LOG_DEBUG("  flags: %" PRIu32, msg->flags);
      break;
  }

  if (msg->request_uuid.size > 0) {
    LOG_DEBUG("  request_uuid: %s", format_hex(msg->request_uuid.bytes, msg->request_uuid.size).c_str());
  }
  if (msg->uuid.size > 0) {
...
void log_vehicle_status(const char *tag, const VCSEC_VehicleStatus *msg) {
  LOG_DEBUG("VCSEC_VehicleStatus:");
  LOG_DEBUG("  has_closureStatuses: %s", msg->has_closureStatuses ? "true" : "false");
  if (msg->has_closureStatuses) {
    LOG_DEBUG("  closureStatuses:");
    LOG_DEBUG("    frontDriverDoor: %s", closure_state_to_string(msg->closureStatuses.frontDriverDoor));
    LOG_DEBUG("    frontPassengerDoor: %s", closure_state_to_string(msg->closureStatuses.frontPassengerDoor));
    LOG_DEBUG("    rearDriverDoor: %s", closure_state_to_string(msg->closureStatuses.rearDriverDoor));
    LOG_DEBUG("    rearPassengerDoor: %s", closure_state_to_string(msg->closureStatuses.rearPassengerDoor));
    LOG_DEBUG("    rearTrunk: %s", closure_state_to_string(msg->closureStatuses.rearTrunk));
    LOG_DEBUG("    frontTrunk: %s", closure_state_to_string(msg->closureStatuses.frontTrunk));
    LOG_DEBUG("    chargePort: %s", closure_state_to_string(msg->closureStatuses.chargePort));
  }
  LOG_DEBUG("  vehicleLockState: %s", vehicle_lock_state_to_string(msg->vehicleLockState));
  LOG_DEBUG("  vehicleSleepStatus: %s", vehicle_sleep_status_to_string(msg->vehicleSleepStatus));
  LOG_DEBUG("  userPresence: %s", user_presence_to_string(msg->userPresence));
}

void log_vssec_signed_message_status(const char *tag, const VCSEC_SignedMessage_status *status) {
  LOG_INFO("  SignedMessage status:");
  LOG_INFO("    counter: %" PRIu32, status->counter);
  LOG_INFO("    signed_message_information: %s",
           vssec_signed_message_information_to_string(status->signedMessageInformation));
}

void log_vssec_whitelist_operation_status(const char *tag, const VCSEC_WhitelistOperation_status *status) {
  LOG_INFO("  WhitelistOperation status:");
  ...
  LOG_INFO("    information: %s",
           vssec_whitelist_operation_information_to_string(status->whitelistOperationInformation));
}

void log_vcsec_command_status(const char *tag, const VCSEC_CommandStatus *msg) {
  LOG_INFO("VCSEC_CommandStatus:");
  LOG_INFO("  commandStatus: %s", vcsec_operation_status_to_string(msg->operationStatus));

  LOG_INFO("  which_sub_message: %d", msg->which_sub_message);
...
void log_charge_state(const char *tag, const CarServer_ChargeState &charge_state) {
  ...
  if (charge_state.has_charging_state) {
    log_charging_state(tag, charge_state.charging_state);
  }
  ...
  if (charge_state.which_optional_battery_range) {
    LOG_DEBUG("Battery Range: %.1f", charge_state.optional_battery_range.battery_range);
  }
...
void log_carserver_response(const char *tag, const CarServer_Response *msg) {
  if (!msg) {
    LOG_DEBUG("CarServerResponse: NULL");
    return;
  }

  LOG_DEBUG("CarServerResponse:");
  LOG_DEBUG("  has_actionStatus: %d", msg->has_actionStatus);
  LOG_DEBUG("  which_response_msg: %d", msg->which_response_msg);
  ...
  switch (msg->which_response_msg) {
    case CarServer_Response_vehicleData_tag:
      ...
      if (msg->response_msg.vehicleData.has_charge_state) {
        log_charge_state(tag, msg->response_msg.vehicleData.charge_state);
      }
      ...
      if (msg->response_msg.vehicleData.has_climate_state) {
        ...
        if (msg->response_msg.vehicleData.climate_state.which_optional_inside_temp_celsius) {
          LOG_DEBUG("    Inside Temp: %.1f",
                    msg->response_msg.vehicleData.climate_state.optional_inside_temp_celsius.inside_temp_celsius);
        }
        if (msg->response_msg.vehicleData.climate_state.which_optional_outside_temp_celsius) {
          LOG_DEBUG("    Outside Temp: %.1f",
                    msg->response_msg.vehicleData.climate_state.optional_outside_temp_celsius.outside_temp_celsius);
        }
        if (msg->response_msg.vehicleData.climate_state.which_optional_is_climate_on) {
          LOG_DEBUG("    HVAC On: %s", msg->response_msg.vehicleData.climate_state.optional_is_climate_on.is_climate_on
                                           ? "true"
                                           : "false");
        }
      }
      ...
      if (msg->response_msg.vehicleData.has_tire_pressure_state) {
        ...
        if (msg->response_msg.vehicleData.tire_pressure_state.which_optional_tpms_pressure_fl) {
          LOG_DEBUG("    Front Left: %.2f",
                    msg->response_msg.vehicleData.tire_pressure_state.optional_tpms_pressure_fl.tpms_pressure_fl);
        }
        if (msg->response_msg.vehicleData.tire_pressure_state.which_optional_tpms_pressure_fr) {
          LOG_DEBUG("    Front Right: %.2f",
                    msg->response_msg.vehicleData.tire_pressure_state.optional_tpms_pressure_fr.tpms_pressure_fr);
        }
        if (msg->response_msg.vehicleData.tire_pressure_state.which_optional_tpms_pressure_rl) {
          LOG_DEBUG("    Rear Left: %.2f",
                    msg->response_msg.vehicleData.tire_pressure_state.optional_tpms_pressure_rl.tpms_pressure_rl);
        }
        if (msg->response_msg.vehicleData.tire_pressure_state.which_optional_tpms_pressure_rr) {
          LOG_DEBUG("    Rear Right: %.2f",
                    msg->response_msg.vehicleData.tire_pressure_state.optional_tpms_pressure_rr.tpms_pressure_rr);
        }
      }

      break;
    case CarServer_Response_getSessionInfoResponse_tag:
      LOG_DEBUG("  getSessionInfoResponse:");
      log_session_info(tag, &msg->response_msg.getSessionInfoResponse);
      break;
    case CarServer_Response_getNearbyChargingSites_tag:
      LOG_DEBUG("  getNearbyChargingSites:");
      break;
