...
int Client::build_universal_message_with_payload(pb_byte_t *payload, size_t payload_length,
  ...
  // Track the request UUID so that if the vehicle replies with a session_info
  // response (e.g. on TIME_EXPIRED / EPOCH-mismatch recovery) the HMAC tag can be
  // verified against the originating request and the local session refreshed.
  // Without this, every signed VCSEC/Infotainment command (Wake, Lock, Climate,
  // Charge, etc.) is unable to recover a stale session.
